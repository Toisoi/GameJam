...
func cover(duration: float = default_cover_duration) -> void:
	if is_running():
		return
	show()
	...
	emit_signal("cover_done")


func uncover(duration: float = default_uncover_duration) -> void:
	if is_running():
		return
